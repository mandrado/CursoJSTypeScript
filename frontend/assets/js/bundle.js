...
form.addEventListener('submit', function (event) {
    ...
    if (checkSendForm(this))
        console.log("Formulário está ok. Pode ser enviado!");
    ;
});
// checar se os password são iguais
function checkEqualPassords(password, password2) {
...
function checkForEmptyFields(...inputs) {
    inputs.forEach(input => {
        if (!input.value)
            showErrorMessage(input, 'Campo não pode fica vazio');
    });
}
// checar se devo enviar os dados do formulário.
